...
| editor-slot page | `workspace.page` union in `src/app/workspace.ts`, a `Show` in `App.tsx` over the editor column (zIndex 60), and a view that takes `width` / `focused` / `blocked` / `onClose` — Settings, LSP status, and all-changes (`ChangesView`, whose `ChangeSection` the controller imports). A page that owns state of its own instead (the commit and comparison views) registers its closer with `workspace.onClosePages`: landing in a file has to take *every* layer over the slot down, and `openFile` only knows about `page` — `closePages()` is what the callers that skip `openFile` for the file already open call |
